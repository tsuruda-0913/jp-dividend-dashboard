...
def ticker_of(code: str) -> str:
    """証券コードを yfinance 用ティッカー（末尾 .T）に変換する。

    「483A」のような英数字混在の新形式コードにも対応。
    """
...
def load_stocks() -> tuple[list[dict], bool]:
    """Google スプレッドシートから銘柄リストと財務指標を読み込む。

    戻り値: (銘柄リスト, フォールバックしたか)。
    シートが読めなかった場合は (FALLBACK_STOCKS, True) を返し、
    アプリ側で警告バナーを出せるようにする。

    各要素の例:
      {"code": "1450", "name": "ＴＡＮＡＫＥＮ", "market": "東S", "finance": "(単)",
       "sector": "建設", "roe": 17.1, "equity_ratio": 76.57,
       "current_ratio": 394.27, "quick_ratio": 393.19, "yield": 4.1,
    ...
    try:
        ...
        market_col = find("市場")
        finance_col = find("財務")
        sector_col = find("セクター") or find("業種")
        roe_col = find("ROE")
            ...
            stocks.append(
                {
                    ...
                    "sector": _text(row[sector_col]) if sector_col else "",
        ...
        return [dict(s) for s in FALLBACK_STOCKS], True
